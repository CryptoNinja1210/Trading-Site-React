# NSE Stock Screener based on DevConnector 2.0

> NSE stock screener for brokers and any users

This is a MERN stack application from the "MERN Stack Front To Back" course on [Udemy](https://www.udemy.com/mern-stack-front-to-back/?couponCode=TRAVERSYMEDIA). It is a small social network app tutorial that includes authentication, profiles and forum posts.
...
routes needed to be inside the original `<section className="container-fluid">`.
To solve this each page component in App.js (any child of a `<Route />`) gets
wrapped in it's own `<section className="container-fluid">`, So we no longer need that
...
NSE Stock Screener v1.0.0
Dev Connector v2.0.0
